    ...
    AdminPermission.findOne({
      ...
      .then((response) => {
        return ResponseHandler.success(
          res,
          "",
          UserTransformer.AdminPermission(response)
        );
      })
      .catch((err) => {
  ...
  store = (req, res) => {
    ...
    if (!errors.isEmpty()) {
      return ResponseHandler.error(
        res,
        422,
        validationLanguage.admin_permission_required,
        errors.array()
      );
    }
    AdminPermission.findOne({
        ...
        if (!response) {
          AdminPermission.create({
            ...
            .then((response) => {
              return ResponseHandler.success(
                res,
                responseLanguage.admin_permission_store_success,
                UserTransformer.AdminPermission(response)
              );
            })
            .catch((err) => {
          ...
          AdminPermission.update(
            ...
            .then((response) => {
              return ResponseHandler.success(
                res,
                responseLanguage.admin_permission_update_success,
                UserTransformer.AdminPermission(response)
              );
            })
            .catch((err) => {
