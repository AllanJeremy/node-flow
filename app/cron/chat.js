    ...
    DeleteChannelMessage.findAll({
      ...
      .then((response) => {
        response.map(async (item) => {
          ...
          try {
            const isDeleted = await serverClient.deleteMessage(
              item["message_id"],
              true
            );
            if (isDeleted) {
  ...
  GetMessages = async () => {
    ExportChannel.findAll().then((response) => {
      response.map(async (item) => {
        ...
        const channelResponse = await serverClient.getExportChannelStatus(
          item.task_id
        );
        https
          .get(channelResponse.result.url, function (res) {
            let data = "",
              json_data = "";
