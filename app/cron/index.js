  ...
  elasticSearch = async (data) => {
    switch (data.action) {
      case ElasticsearchEventsAction.createUser:
        return await ElasticSearchHandler.addDocument(
          data.metadata.id,
          data.metadata
        );
        break;
      case ElasticsearchEventsAction.updateUser:
        return await ElasticSearchHandler.updateDocumentField(
          data.metadata.id,
          data.metadata
        );
        break;
      ...
      case ElasticsearchEventsAction.raceRenamed:
        return await ElasticSearchHandler.renameDocumentField(
          "race",
          data.metadata
        );
      ...
      case ElasticsearchEventsAction.healthCategoryRenamed:
        return await ElasticSearchHandler.renameDocumentListItem(
          "health_categories",
          data.metadata
        );
        break;
      ...
      case ElasticsearchEventsAction.raceDelete:
        return await ElasticSearchHandler.deleteDocumentField(
          "race",
          data.metadata.name
        );
      ...
      case ElasticsearchEventsAction.healthCategoryDelete:
        return await ElasticSearchHandler.deleteItemFromDocumentList(
          "health_categories",
          data.metadata.name
        );
      ...
      ElasticsearchEventsHandler.list()
        .then((response) => {
          response.map(async (item, index) => {
            if (item.attempted < 3) {
              this.elasticSearch(item.dataValues)
                .then((res) => {
                  if (res.statusCode == 200 || res.statusCode == 201) {
                    ElasticsearchEventsHandler.destroy(item.id);
                  } else {
                    ElasticsearchEventsHandler.update(
                      item.id,
                      res,
                      item.attempted
                    );
